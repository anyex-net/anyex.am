...
//CustomMdSpi* MdHttp::sm_pMdSpi;
vector<pair<const char *, CustomMdSpi *>> MdHttp::sm_vMd;
...
//void MdHttp::setMdSpi(CustomMdSpi *mdSpi) {
//    sm_pMdSpi = mdSpi;
//}

void MdHttp::SetMdVector(vector<pair<const char *, CustomMdSpi *>> md) {
    sm_vMd = md;
}
...
bool MdHttp::SubscribeMarketData(HttpRequest& req, HttpResponse& res)
{
    ...
    params[1] = req.getHeader("ppInstrumentID");
    if (NULL == params[1]) {
        params_flag = false;
        logger_error("ppInstrumentID is null");
        write_buf.append("ppInstrumentID is null");
    } else {
        logger_debug(DEBUG_TEST2, 1, "ppInstrumentID=%s", params[1]);
    }

    if (params_flag) {
        bool md_vector_flag = false;
        for(auto v = sm_vMd.begin(); v != sm_vMd.end(); ++v) {
            if (strcmp(v->first, params[0]) == 0) {
                CustomMdSpi* mdSpi = (CustomMdSpi*)v->second;
                logger("订阅合约:%s", params[1]);
                int rt = mdSpi->subscribeMarketData(params[1], 0);
...
//                logger("nCount=%d", nCount);
                int rt = mdSpi->unSubscribeMarketData(params[1], 0);
                write_buf.format("%d", rt);
                md_vector_flag = true;
                break;
            }
