...
bool CtpUtil::GetHostInfo(acl::string& HostName, acl::string& IP) {
    char name[256];
...
char* CtpUtil::CtpConvert(char *value, size_t len) {
...
//    char outbuf[outlen];
    ...
    CtpUtil::CodeConvert(encFrom, encTo, value, len, outbuf, &outlen);
    return outbuf;
}

int CtpUtil::CodeConvert(char *fromCode, char *toCode, char *inbuf, size_t len, char *outbuf, size_t *outlen) {
...
void CtpUtil::KafkaOrRedis(const char *TOPIC_NAME, char *value, size_t len) {
...
bool CtpUtil::CreateDirs(const string& dirName)
{
