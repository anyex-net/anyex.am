...
std::map<acl::string, std::shared_ptr<Account>> ACCOUNTS;
rd_kafka_t *rk;
...
void init::initConfiguration()
{
    ...
    const char *filepath = "conf/esunnyServer.conf";
...
void init::connectToDatabase()
{
    ...
    acl::query query;
    query.create_sql("select accountNo,accountPassword,appID,authCode,quoteApiAddr,tradeApiAddr"
                     " from EsNpfuturesAccountInfo where 1=1");
    ...
    else
    {
        ...
        for (size_t i=0; i<db.length(); i++)
        {
            ...
            if (accountNo_ptr == NULL || password_ptr == NULL || authCode_ptr == NULL || appID_ptr == NULL
                || quoteApiAddr_ptr == NULL || quoteApiAddr_ptr == NULL || (strcmp(accountNo_ptr, "") == 0)
                || (strcmp(password_ptr, "") == 0) || (strcmp(authCode_ptr, "") == 0) || (strcmp(appID_ptr, "") == 0)
            ...
            account->setAccountNo(accountNo_ptr);
            account->setPassword(password_ptr);
            account->setAppID(appID_ptr);
            account->setAuthCode(authCode_ptr);
            account->setIp(trade_ip);
            account->setPort(trade_port);
            account->setQuoteIp(quote_ip);
            account->setQuotePort(quote_port);
            // 把账户放入全局变量中
            ACCOUNTS.insert(std::make_pair(accountNo_ptr, account));
        }
...
//    auto account = std::make_shared<Account>();
//    account->setAccountNo("Q578432149");
//    account->setPassword("12345678");
//    account->setAuthCode("Demo_TestCollect");
//    account->setAppID("Demo_TestCollect");
//    account->setIp("123.161.206.213");
//    account->setPort(6160);
//    account->setQuoteIp("123.161.206.213");
//    account->setQuotePort(6161);
//    ACCOUNTS.insert(std::make_pair("Q578432149", account));
}
