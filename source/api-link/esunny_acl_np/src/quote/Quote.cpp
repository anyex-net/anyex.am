...
const char* TOPIC_SUBQUOTE = "NpTopicSubscribeQuoteWhole";
const char* TOPIC_UNSUBQUOTE = "NpTopicUnSubscribeQuoteWhole";
...
int Quote::RunQuote()
{
    logger("%s quote start", m_pAccount->getAccountNo());
    if(NULL == m_pAPI) {
        logger_error("Error: m_pAPI is NULL.");
        return -19002;
    }
//    logger("ip: %s, port: %d ", m_pAccount->getQuoteIp(), m_pAccount->getQuotePort());
    TAPIINT32 iErr = TAPIERROR_SUCCEED;
    //设定服务器IP、端口
    if (this->m_reasonCode == 0) {
        iErr = m_pAPI->SetHostAddress(m_pAccount->getQuoteIp(), m_pAccount->getQuotePort());
...
void Quote::loopCmd()
{
    ...
    while(1)
    {
        ...
        switch (cmd)
        {
            ...
            case '3': this->q_subscribeQuote("DCE", "M", "2107");break;
            case '4': this->q_unSubscribeQuote("DCE", "M", "2107");break;
...
int Quote::q_qryContract(const char* ExchangeNo)
{
    ...
    strcpy(com.ExchangeNo, "DCE");
	strcpy(com.CommodityNo, "M");
...
{
    logger_error("API断开,断开原因:: %d, AccountNo %s", reasonCode, m_pAccount->getAccountNo());
