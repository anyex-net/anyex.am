...
void QuoteHttp::start(const char* addrs) {
    MasterService ms;
...
bool QuoteHttp::SubscribeQuoteWhole(HttpRequest& req, HttpResponse& res)
{
    logger("订阅行情");
    ...
    if (bParams) {
        bool quote_threads_vector = false;
        for(auto v = quote_vector.begin(); v != quote_vector.end(); ++v)
        {
            if (strcmp(params[0], (v->first).c_str()) == 0)
            {
                ...
                quote_threads_vector = true;
                break;
            }
        }
        if (!quote_threads_vector)
        {
...
bool QuoteHttp::UnSubscribeQuoteWhole(HttpRequest& req, HttpResponse& res)
{
    logger("取消订阅");
...
bool QuoteHttp::ConnectQuote(HttpRequest& req, HttpResponse& res)
{
    logger("连接行情");
//    // 获取 body 中的数据，如果没有消费HTTP BODY数据，会导致报错
    acl::string write_buf;
...
bool QuoteHttp::DisConnectQuote(HttpRequest& req, HttpResponse& res)
{
    logger("断开连接");
//    // 获取 body 中的数据，如果没有消费HTTP BODY数据，会导致报错
    acl::string write_buf;
