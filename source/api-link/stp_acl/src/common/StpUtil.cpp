...
bool StpUtil::GetHostInfo(std::string& hostName, std::string& Ip)
{
    char name[256];
...
bool StpUtil::CreateDirs(const std::string& dirName)
{
...
static bool isGBK(char* data, int len)
{
    int i  = 0;
    while (i < len)  {
        if (data[i] <= 0x7f) {
...
char* StpUtil::GB18030ToUTF8(char *value, size_t len)
{
  ...
  *           IGNORE ：遇到无法转换字符跳过*/
    ...
    StpUtil::CodeConvert(encFrom, encTo, value, len, outbuf, &outlen);
    return outbuf;
}

int StpUtil::CodeConvert(char *fromCode, char *toCode, char *inbuf, size_t len, char *outbuf, size_t *outlen)
{
    iconv_t cd = iconv_open(toCode, fromCode);

    if (cd == (iconv_t)-1)
    {
        logger_error("iconv_open is false");
        return -1;
    }
    ...
    if (iconv(cd, &tmpin, &inlen, &tmpout, outlen) == -1) {
        switch(errno)
        {
            ...
            case EINVAL:
                ...
                break;
            default:
                logger("CodeConvert failed, %s", errno);
        }
...
void StpUtil::KafkaOrRedis(const char *TOPIC_NAME, char *value, size_t len)
{
