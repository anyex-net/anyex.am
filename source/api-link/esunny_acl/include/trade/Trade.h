...
public:
    ...
    // order
    int t_insertOrder(const char* ExchangeNo, const char* CommodityNo,
    ...
    int t_canncelOrder(const char* OrderNo);
    void t_modify();
    void t_activeOrder();
    ...
    int t_qryFund();   // 查询客户资金
    void t_qryTradeMessage();    // 获取交易或风控消息

    // Commodity、Contract
    int t_qryCommodity();
    int t_qryContract(const char *ExchangeNo, const char *CommodityNo);

    // UserTrade
    int t_qryOrder(); // 查询委托信息
    int t_qryOrderByOrderNo( const char* OrderNo);
    void t_qryOrderProcess(); // 查询委托变化流程
    int t_qryFill(); // 查询成交信息
    int t_qryFillByOrderNo(const char* OrderNo);
    int  t_qryPosition(); // 查询用户持仓
    int t_qryPositionSummary(); // 查询用户持仓汇总
    ...
    // Rent
    int t_qryAccountFeeRent(); // 客户手续费计算参数查询
    int t_qryAccountMarginRent(); // 客户保证金计算参数查询
