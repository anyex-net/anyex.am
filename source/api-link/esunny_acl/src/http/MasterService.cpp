...
acl::master_int_tbl var_conf_int_tab[] = {
        { "rw_timeout", -1, &var_cfg_rw_timeout, 0, 0 },
...
MasterService::MasterService(void)
{
    redis_   = NULL;
...
HttpService& MasterService::get_service(void) const
{
    return *service_;
}

bool MasterService::thread_on_read(acl::socket_stream* conn)
{
    HttpServlet* servlet = (HttpServlet*) conn->get_ctx();
    if (servlet == NULL) {
...
bool MasterService::thread_on_accept(acl::socket_stream* conn)
{
    ...
    HttpServlet* servlet = new HttpServlet(*service_, conn, session);
    conn->set_ctx(servlet);

    return true;
}

bool MasterService::thread_on_timeout(acl::socket_stream* conn)
{
...
void MasterService::thread_on_close(acl::socket_stream* conn)
{
    logger_error("disconnect from %s, fd: %d", conn->get_peer(),
           conn->sock_handle());
...
void MasterService::thread_on_init(void)
{
}

void MasterService::thread_on_exit(void)
{
}

void MasterService::proc_on_listen(acl::server_socket& ss)
{
    logger(">>>listen %s ok<<<", ss.get_addr());
}

void MasterService::proc_on_init(void)
{
...
void MasterService::proc_on_exit(void)
{
    delete redis_;
}

bool MasterService::proc_exit_timer(size_t nclients, size_t nthreads)
{
    if (nclients == 0) {
...
bool MasterService::proc_on_sighup(acl::string&)
{
