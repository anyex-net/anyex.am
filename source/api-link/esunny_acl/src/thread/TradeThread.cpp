...
void TradeThread::init()
{
    /// 初始化交易
    //取得API的版本信息
    logger("TradeApi的版本信息: %s", GetITapTradeAPIVersion());

    //创建API实例
    TAPIINT32 iResult = TAPIERROR_SUCCEED;
    ITapTrade::TapAPIApplicationInfo tradeAppInfo;
    strcpy(tradeAppInfo.AuthCode, TRADE_ACCOUNTS[this->AccountNo]->getAuthCode());
    // 关键操作日志路径 APIRun日志
    strcpy(tradeAppInfo.KeyOperationLogPath, "logs/TradeAPIRunLog");
    tradeAppInfo.LogLevel = APILOGLEVEL_ERROR;
    ...
    objTrade.setAccount(TRADE_ACCOUNTS[this->AccountNo]);
}
