<template>
  <div class="app-container">
    ...
    <el-table
      v-loading="npSpotAccountInfoLoading"
      :data="npSpotAccountInfoData"
    ...
    <el-dialog
      title="内盘现货投资者帐号表管理"
      :visible.sync="npSpotAccountInfoDialog"
      :close-on-click-modal="false"
      width="600"
    >
      <el-form
        ref="npSpotAccountInfoForm"
        :model="npSpotAccountInfoForm"
        :rules="npSpotAccountInfoRules"
        label-width="150px"
        class="npSpotAccountInfoForm"
      >
        <el-form-item label="AM平台账户ID" prop="accountId">
          <el-input v-model="npSpotAccountInfoForm.accountId" placeholder="请输入AM平台账户ID" />
        </el-form-item>
        <el-form-item label="经纪公司代码" prop="brokerID">
          <el-input v-model="npSpotAccountInfoForm.brokerID" placeholder="请输入经纪公司代码" />
        </el-form-item>
        <el-form-item label="投资者帐号" prop="investorID">
          <el-input v-model="npSpotAccountInfoForm.investorID" placeholder="请输入投资者帐号" />
        </el-form-item>
        <el-form-item label="投资者密码" prop="investorPassword">
          <el-input v-model="npSpotAccountInfoForm.investorPassword" placeholder="请输入投资者密码" />
        </el-form-item>
        <el-form-item label="认证码" prop="authCode">
          <el-input v-model="npSpotAccountInfoForm.authCode" placeholder="请输入认证码" />
        </el-form-item>
        <el-form-item label="App代码" prop="appID">
          <el-input v-model="npSpotAccountInfoForm.appID" placeholder="请输入App代码" />
        </el-form-item>
        <el-form-item label="行情前置机地址" prop="mdFrontAddr">
          <el-input v-model="npSpotAccountInfoForm.mdFrontAddr" placeholder="请输入行情前置机地址" />
        </el-form-item>
        <el-form-item label="交易前置机地址" prop="tradeFrontAddr">
          <el-input v-model="npSpotAccountInfoForm.tradeFrontAddr" placeholder="请输入交易前置机地址" />
        </el-form-item>
        <el-form-item>
          <el-button type="success" @click="doSubmit('npSpotAccountInfoForm')">提交</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
  </div>
...
<script>
export default {
  name: 'NpSpotAccountInfoName',
  data() {
    return {
      npSpotAccountInfoLoading: true,
      npSpotAccountInfoDialog: false,
      npSpotAccountInfoData: [],
      commodityTypeSelects: [],
      dicts: [],
      npSpotAccountInfoForm: {
        'id': '',
        'accountId': '',
        'brokerID': '',
        'investorID': '',
  ...
  methods: {
    ...
    doInitData() {
      this.$http({
        url: '/npspot/dict/npSpotAccountInfo',
        method: 'get'
      }).then(res => {
        if (res.code === 200) {
    ...
    doSearch: function(data, type) {
      ...
      this.npSpotAccountInfoLoading = true;
      this.$http({
        url: '/npspot/npSpotAccountInfo/data',
        method: 'post',
        data: Object.assign(this.pageParams, this.searchForm)
      }).then(res => {
        if (res.code === 200) {
          this.npSpotAccountInfoData = res.rows;
          this.pageParams.totalPage = res.totalPage;
          this.pageParams.total = res.total;
          this.npSpotAccountInfoLoading = false;
        } else {
    ...
    dialogAdd: function() {
      ...
      this.npSpotAccountInfoDialog = true;
    },
    dialogEdit: function(row, column) {
      if (this.$refs.npSpotAccountInfoForm) {
        this.$refs.npSpotAccountInfoForm.resetFields();
      }
      this.$http({
        url: '/npspot/npSpotAccountInfo/findBy',
        method: 'get',
        params: {
          'id': column.id
        }
    ...
    doSubmit: function(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$http({
            url: '/npspot/npSpotAccountInfo/save',
            method: 'post',
            data: this.npSpotAccountInfoForm
          }).then(res => {
            if (res.code === 200) {
          ...
          this.npSpotAccountInfoDialog = false;
        }
      });
    },
    doDelete: function(row, column) {
      this.$confirm('确认删除该记录吗, 是否继续?', '提示', {
        ...
        this.$http({
          url: '/npspot/npSpotAccountInfo/del',
          method: 'post',
          data: {
            ids: column.id
          }
