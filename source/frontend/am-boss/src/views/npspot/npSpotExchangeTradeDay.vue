<template>
  <div class="app-container">
    ...
    <el-table
      v-loading="npSpotExchangeTradeDayLoading"
      :data="npSpotExchangeTradeDayData"
    ...
    <el-dialog
      title="内盘现货交易所交易日表管理"
      :visible.sync="npSpotExchangeTradeDayDialog"
      :close-on-click-modal="false"
      width="600"
    >
      <el-form
        ref="npSpotExchangeTradeDayForm"
        :model="npSpotExchangeTradeDayForm"
        :rules="npSpotExchangeTradeDayRules"
        label-width="150px"
        class="npSpotExchangeTradeDayForm"
      >
        <el-form-item label="交易所代码" prop="exchangeID">
          <el-input v-model="npSpotExchangeTradeDayForm.exchangeID" placeholder="请输入交易所代码" />
        </el-form-item>
        <el-form-item label="产品代码" prop="productID">
          <el-input v-model="npSpotExchangeTradeDayForm.productID" placeholder="请输入产品代码" />
        </el-form-item>
        <el-form-item label="星期几(1~7)" prop="weekNum">
          <el-input v-model="npSpotExchangeTradeDayForm.weekNum" placeholder="请输入星期几(1~7)" />
        </el-form-item>
        <el-form-item label="是否交易日" prop="isTradeDay">
          <el-select v-model="npSpotExchangeTradeDayForm.isTradeDay" placeholder="请输入是否交易日">
            <el-option :key="0" label="非交易日" :value="0" />
            <el-option :key="1" label="交易日" :value="1" />
          </el-select>
        </el-form-item>
        <el-form-item label="交易日零点时间戳" prop="tradeDayTime">
          <el-input v-model="npSpotExchangeTradeDayForm.tradeDayTime" placeholder="请输入交易日零点时间戳" />
        </el-form-item>
        <el-form-item label="交易日开盘时间戳" prop="tradeDayOpenTime">
          <el-input v-model="npSpotExchangeTradeDayForm.tradeDayOpenTime" placeholder="请输入交易日开盘时间戳" />
        </el-form-item>
        <el-form-item label="交易日收盘时间戳" prop="tradeDayCloseTime">
          <el-input v-model="npSpotExchangeTradeDayForm.tradeDayCloseTime" placeholder="请输入交易日收盘时间戳" />
        </el-form-item>
        <el-form-item>
          <el-button type="success" @click="doSubmit('npSpotExchangeTradeDayForm')">提交</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
  </div>
...
export default {
  ...
  data() {
    return {
      npSpotExchangeTradeDayLoading: true,
      npSpotExchangeTradeDayDialog: false,
      npSpotExchangeTradeDayData: [],
      productID: [],
      npSpotExchangeTradeDayForm: {
        'id': '',
        'exchangeID': '',
        'productID': '',
        'weekNum': '',
  ...
  methods: {
    ...
    doSearch: function(data, type) {
      ...
      this.npSpotExchangeTradeDayLoading = true;
      this.$http({
        url: '/npspot/npSpotExchangeTradeDay/data',
        method: 'post',
        data: Object.assign(this.pageParams, this.searchForm)
      }).then(res => {
        if (res.code === 200) {
          this.npSpotExchangeTradeDayData = res.rows;
          this.pageParams.totalPage = res.totalPage;
          this.pageParams.total = res.total;
          this.npSpotExchangeTradeDayLoading = false;
        } else {
    ...
    dialogAdd: function() {
      this.npSpotExchangeTradeDayForm = {
        'id': '',
        'exchangeID': '',
        'productID': '',
        'weekNum': '',
      ...
      this.npSpotExchangeTradeDayDialog = true;
    },
    dialogEdit: function(row, column) {
      if (this.$refs.npSpotExchangeTradeDayForm) {
        this.$refs.npSpotExchangeTradeDayForm.resetFields();
      }
      this.$http({
        url: '/npspot/npSpotExchangeTradeDay/findBy',
        method: 'get',
        params: {
          'id': column.id
        }
    ...
    doSubmit: function(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$http({
            url: '/npspot/npSpotExchangeTradeDay/save',
            method: 'post',
            data: this.npSpotExchangeTradeDayForm
          }).then(res => {
            if (res.code === 200) {
          ...
          this.npSpotExchangeTradeDayDialog = false;
        }
      });
    },
    doDelete: function(row, column) {
      this.$confirm('确认删除该记录吗, 是否继续?', '提示', {
        ...
        this.$http({
          url: '/npspot/npSpotExchangeTradeDay/del',
          method: 'post',
          data: {
            ids: column.id
          }
