<template>
  <div class="app-container">
    <el-form ref="searchForm" :model="searchForm" :inline="true" size="mini">
      ...
      <el-form-item label="资金账户代码(对应AccountID)">
        <el-input v-model="searchForm.accountNo" clearable placeholder="请输入资金账户代码(对应AccountID)"></el-input>
      </el-form-item>
      <el-form-item label="资金账户所属投资者代码">
        <el-input v-model="searchForm.investorID" clearable placeholder="请输入资金账户所属投资者代码"></el-input>
      </el-form-item>
      <el-form-item>
    ...
    <el-table
      v-loading="stpNpSpotTradingAccountLoading"
      :data="stpNpSpotTradingAccountData"
    ...
    >
      ...
      <el-table-column prop="investorID" label="资金账户所属投资者代码" />
      ...
      <el-table-column prop="accountType" label="资金账户类型" />
      <el-table-column prop="bankID" label="银行代码" />
    ...
    <el-dialog
      title="内盘现货资金账户表管理"
      :visible.sync="stpNpSpotTradingAccountDialog"
      :close-on-click-modal="false"
      width="600"
    >
      <el-form
        ref="stpNpSpotTradingAccountForm"
        :model="stpNpSpotTradingAccountForm"
        label-width="260px"
        class="stpNpSpotTradingAccountForm"
      >
        <el-form-item label="AM平台账户ID" prop="accountId">
          <el-input v-model="stpNpSpotTradingAccountForm.accountId" placeholder="请输入AM平台账户ID" />
        </el-form-item>
        <el-form-item label="经纪公司代码" prop="brokerID">
          <el-input v-model="stpNpSpotTradingAccountForm.brokerID" placeholder="请输入经纪公司代码" />
        </el-form-item>
        <el-form-item label="经纪公司部门代码" prop="departmentID">
          <el-input v-model="stpNpSpotTradingAccountForm.departmentID" placeholder="请输入经纪公司部门代码" />
        </el-form-item>
        <el-form-item label="资金账户代码(对应AccountID)" prop="accountNo">
          <el-input v-model="stpNpSpotTradingAccountForm.accountNo" placeholder="请输入资金账户代码(对应AccountID)" />
        </el-form-item>
        <el-form-item label="资金账户所属投资者代码" prop="investorID">
          <el-input v-model="stpNpSpotTradingAccountForm.investorID" placeholder="请输入资金账户所属投资者代码" />
        </el-form-item>
        <el-form-item label="币种代码" prop="currencyID">
          <el-input v-model="stpNpSpotTradingAccountForm.currencyID" placeholder="请输入币种代码" />
        </el-form-item>
        <el-form-item label="上日结存" prop="preDeposit">
          <el-input v-model="stpNpSpotTradingAccountForm.preDeposit" placeholder="请输入上日结存" />
        </el-form-item>
        <el-form-item label="可用资金" prop="usefulMoney">
          <el-input v-model="stpNpSpotTradingAccountForm.usefulMoney" placeholder="请输入可用资金" />
        </el-form-item>
        <el-form-item label="可取资金" prop="fetchLimit">
          <el-input v-model="stpNpSpotTradingAccountForm.fetchLimit" placeholder="请输入可取资金" />
        </el-form-item>
        <el-form-item label="上日未交收金额(港股通专用字段)" prop="preUnDeliveredMoney">
          <el-input v-model="stpNpSpotTradingAccountForm.preUnDeliveredMoney" placeholder="请输入上日未交收金额(港股通专用字段)" />
        </el-form-item>
        <el-form-item label="可用未交收金额(港股通专用字段)" prop="unDeliveredMoney">
          <el-input v-model="stpNpSpotTradingAccountForm.unDeliveredMoney" placeholder="请输入可用未交收金额(港股通专用字段)" />
        </el-form-item>
        <el-form-item label="当日入金金额" prop="deposit">
          <el-input v-model="stpNpSpotTradingAccountForm.deposit" placeholder="请输入当日入金金额" />
        </el-form-item>
        <el-form-item label="当日出金金额" prop="withdraw">
          <el-input v-model="stpNpSpotTradingAccountForm.withdraw" placeholder="请输入当日出金金额" />
        </el-form-item>
        <el-form-item label="冻结的资金(港股通该字段不包括未交收部分冻结资金)" prop="frozenCash">
          <el-input v-model="stpNpSpotTradingAccountForm.frozenCash" placeholder="请输入冻结的资金(港股通该字段不包括未交收部分冻结资金)" />
        </el-form-item>
        <el-form-item label="冻结未交收金额(港股通专用)" prop="unDeliveredFrozenCash">
          <el-input v-model="stpNpSpotTradingAccountForm.unDeliveredFrozenCash" placeholder="请输入冻结未交收金额(港股通专用)" />
        </el-form-item>
        <el-form-item label="冻结的手续费(港股通该字段不包括未交收部分冻结手续费)" prop="frozenCommission">
          <el-input
            v-model="stpNpSpotTradingAccountForm.frozenCommission"
            placeholder="请输入冻结的手续费(港股通该字段不包括未交收部分冻结手续费)"
          />
        </el-form-item>
        <el-form-item label="冻结未交收手续费(港股通专用)" prop="unDeliveredFrozenCommission">
          <el-input v-model="stpNpSpotTradingAccountForm.unDeliveredFrozenCommission" placeholder="请输入冻结未交收手续费(港股通专用)" />
        </el-form-item>
        <el-form-item label="手续费(港股通该字段不包括未交收部分手续费)" prop="commission">
          <el-input v-model="stpNpSpotTradingAccountForm.commission" placeholder="请输入手续费(港股通该字段不包括未交收部分手续费)" />
        </el-form-item>
        <el-form-item label="占用未交收手续费(港股通专用)" prop="unDeliveredCommission">
          <el-input v-model="stpNpSpotTradingAccountForm.unDeliveredCommission" placeholder="请输入占用未交收手续费(港股通专用)" />
        </el-form-item>
        <el-form-item label="资金账户类型" prop="accountType">
          <el-input v-model="stpNpSpotTradingAccountForm.accountType" placeholder="请输入资金账户类型" />
        </el-form-item>
        <el-form-item label="银行代码" prop="bankID">
          <el-input v-model="stpNpSpotTradingAccountForm.bankID" placeholder="请输入银行代码" />
        </el-form-item>
        <el-form-item label="银行账户" prop="bankAccountID">
          <el-input v-model="stpNpSpotTradingAccountForm.bankAccountID" placeholder="请输入银行账户" />
        </el-form-item>
        <el-form-item label="权利金收入(两融专用)" prop="royaltyIn">
          <el-input v-model="stpNpSpotTradingAccountForm.royaltyIn" placeholder="请输入权利金收入(两融专用)" />
        </el-form-item>
        <el-form-item label="权利金支出(两融专用)" prop="royaltyOut">
          <el-input v-model="stpNpSpotTradingAccountForm.royaltyOut" placeholder="请输入权利金支出(两融专用)" />
        </el-form-item>
        <el-form-item label="融券卖出金额(两融专用)" prop="creditSellAmount">
          <el-input v-model="stpNpSpotTradingAccountForm.creditSellAmount" placeholder="请输入融券卖出金额(两融专用)" />
        </el-form-item>
        <el-form-item label="融券卖出使用金额(用于偿还融资负债或买特殊品种的金额)(两融专用)" prop="creditSellUseAmount">
          <el-input
            v-model="stpNpSpotTradingAccountForm.creditSellUseAmount"
            placeholder="请输入融券卖出使用金额(用于偿还融资负债或买特殊品种的金额)(两融专用)"
          />
        </el-form-item>
        <el-form-item label="虚拟资产(两融专用)" prop="virtualAssets">
          <el-input v-model="stpNpSpotTradingAccountForm.virtualAssets" placeholder="请输入虚拟资产(两融专用)" />
        </el-form-item>
        <el-form-item label="融券卖出金额冻结(用于偿还融资负债或买特殊品种的未成交冻结金额)(两融专用)" prop="creditSellFrozenAmount">
          <el-input
            v-model="stpNpSpotTradingAccountForm.creditSellFrozenAmount"
            placeholder="请输入融券卖出金额冻结(用于偿还融资负债或买特殊品种的未成交冻结金额)(两融专用)"
          />
        </el-form-item>
<!--        <el-form-item>-->
<!--          <el-button type="success" @click="doSubmit('stpNpSpotTradingAccountForm')">提交</el-button>-->
<!--        </el-form-item>-->
      </el-form>
    </el-dialog>
  </div>
...
<script>
export default {
  name: 'StpNpSpotTradingAccountName',
  data() {
    return {
      stpNpSpotTradingAccountLoading: true,
      stpNpSpotTradingAccountDialog: false,
      stpNpSpotTradingAccountData: [],
      commodityTypeSelects: [],
      dicts: [],
      stpNpSpotTradingAccountForm: {
        'id': '',
        'accountId': '',
        'brokerID': '',
        'departmentID': '',
      ...
      stpNpSpotTradingAccountRules: {
        ...
        investorID: [
          { required: true, message: '资金账户所属投资者代码不可为空', trigger: 'change' }
        ],
        currencyID: [
  ...
  methods: {
    ...
    doSearch: function(data, type) {
      ...
      this.stpNpSpotTradingAccountLoading = true;
      this.$http({
        url: '/npspot/stpNpSpotTradingAccount/data',
        method: 'post',
        data: Object.assign(this.pageParams, this.searchForm)
      }).then(res => {
        if (res.code === 200) {
          this.stpNpSpotTradingAccountData = res.rows;
          this.pageParams.totalPage = res.totalPage;
          this.pageParams.total = res.total;
          this.stpNpSpotTradingAccountLoading = false;
        } else {
    ...
    dialogAdd: function() {
      this.stpNpSpotTradingAccountForm = {
        'id': '',
        'accountId': '',
        'brokerID': '',
        'departmentID': '',
      ...
      this.stpNpSpotTradingAccountDialog = true;
    },
    dialogEdit: function(row, column) {
      if (this.$refs.stpNpSpotTradingAccountForm) {
        this.$refs.stpNpSpotTradingAccountForm.resetFields();
      }
      this.$http({
        url: '/npspot/stpNpSpotTradingAccount/findBy',
        method: 'get',
        params: {
          'id': column.id
        }
    ...
    doSubmit: function(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$http({
            url: '/npspot/stpNpSpotTradingAccount/save',
            method: 'post',
            data: this.stpNpSpotTradingAccountForm
          }).then(res => {
            if (res.code === 200) {
          ...
          this.stpNpSpotTradingAccountDialog = false;
        }
      });
    },
    doDelete: function(row, column) {
      this.$confirm('确认删除该记录吗, 是否继续?', '提示', {
        ...
        this.$http({
          url: '/npspot/stpNpSpotTradingAccount/del',
          method: 'post',
          data: {
            ids: column.id
          }
