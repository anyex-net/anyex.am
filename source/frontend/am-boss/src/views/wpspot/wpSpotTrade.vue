...
<!--        <el-button type="success" icon="el-icon-circle-plus" @click="dialogAdd()">添加</el-button>-->
    ...
    <el-table v-loading="wpSpotTradeLoading" :data="wpSpotTradeData" style="width:100%;margin-bottom:20px;" border row-key="id">
...
<!--      <el-table-column prop="investUnitID" label="投资单元代码" />-->
    ...
    />
    <el-dialog title="外盘现货成交表管理" :visible.sync="wpSpotTradeDialog" :close-on-click-modal="false" width="600">
      <el-form ref="wpSpotTradeForm" :model="wpSpotTradeForm" :rules="wpSpotTradeRules" label-width="150px" class="wpSpotTradeForm">
        <el-form-item label="主键" prop="id">
          <el-input v-model="wpSpotTradeForm.id" placeholder="请输入主键" />
        </el-form-item>
        <el-form-item label="AM平台账户ID" prop="accountId">
          <el-input v-model="wpSpotTradeForm.accountId" placeholder="请输入AM平台账户ID" />
        </el-form-item>
        <el-form-item label="经纪公司代码" prop="brokerID">
          <el-input v-model="wpSpotTradeForm.brokerID" placeholder="请输入经纪公司代码" />
        </el-form-item>
        <el-form-item label="投资者代码" prop="investorID">
          <el-input v-model="wpSpotTradeForm.investorID" placeholder="请输入投资者代码" />
        </el-form-item>
        <el-form-item label="合约代码" prop="instrumentID">
          <el-input v-model="wpSpotTradeForm.instrumentID" placeholder="请输入合约代码" />
        </el-form-item>
        <el-form-item label="报单引用" prop="orderRef">
          <el-input v-model="wpSpotTradeForm.orderRef" placeholder="请输入报单引用" />
        </el-form-item>
        <el-form-item label="用户代码" prop="userID">
          <el-input v-model="wpSpotTradeForm.userID" placeholder="请输入用户代码" />
        </el-form-item>
        <el-form-item label="交易所代码" prop="exchangeID">
          <el-input v-model="wpSpotTradeForm.exchangeID" placeholder="请输入交易所代码" />
        </el-form-item>
        <el-form-item label="成交编号" prop="tradeID">
          <el-input v-model="wpSpotTradeForm.tradeID" placeholder="请输入成交编号" />
        </el-form-item>
        <el-form-item label="买卖方向" prop="direction">
          <el-input v-model="wpSpotTradeForm.direction" placeholder="请输入买卖方向" />
        </el-form-item>
        <el-form-item label="报单编号" prop="orderSysID">
          <el-input v-model="wpSpotTradeForm.orderSysID" placeholder="请输入报单编号" />
        </el-form-item>
        <el-form-item label="会员代码" prop="participantID">
          <el-input v-model="wpSpotTradeForm.participantID" placeholder="请输入会员代码" />
        </el-form-item>
        <el-form-item label="客户代码" prop="clientID">
          <el-input v-model="wpSpotTradeForm.clientID" placeholder="请输入客户代码" />
        </el-form-item>
        <el-form-item label="交易角色(交易角色类型数据字典)" prop="tradingRole">
          <el-input v-model="wpSpotTradeForm.tradingRole" placeholder="请输入交易角色" />
        </el-form-item>
        <el-form-item label="合约在交易所的代码" prop="exchangeInstID">
          <el-input v-model="wpSpotTradeForm.exchangeInstID" placeholder="请输入合约在交易所的代码" />
        </el-form-item>
        <el-form-item label="开平标志" prop="offsetFlag">
          <el-input v-model="wpSpotTradeForm.offsetFlag" placeholder="请输入开平标志" />
        </el-form-item>
        <el-form-item label="投机套保标志" prop="hedgeFlag">
          <el-input v-model="wpSpotTradeForm.hedgeFlag" placeholder="请输入投机套保标志" />
        </el-form-item>
        <el-form-item label="价格" prop="price">
          <el-input v-model="wpSpotTradeForm.price" placeholder="请输入价格" />
        </el-form-item>
        <el-form-item label="数量" prop="volume">
          <el-input v-model="wpSpotTradeForm.volume" placeholder="请输入数量" />
        </el-form-item>
        <el-form-item label="成交日期" prop="tradeDate">
          <el-input v-model="wpSpotTradeForm.tradeDate" placeholder="请输入成交日期" />
        </el-form-item>
        <el-form-item label="成交时间" prop="tradeTime">
          <el-input v-model="wpSpotTradeForm.tradeTime" placeholder="请输入成交时间" />
        </el-form-item>
        <el-form-item label="成交类型(成交类型类型数据字典)" prop="tradeType">
          <el-input v-model="wpSpotTradeForm.tradeType" placeholder="请输入成交类型" />
        </el-form-item>
        <el-form-item label="成交价来源(成交价来源类型数据字典)" prop="priceSource">
          <el-input v-model="wpSpotTradeForm.priceSource" placeholder="请输入成交价来源" />
        </el-form-item>
        <el-form-item label="交易所交易员代码" prop="traderID">
          <el-input v-model="wpSpotTradeForm.traderID" placeholder="请输入交易所交易员代码" />
        </el-form-item>
        <el-form-item label="本地报单编号" prop="orderLocalID">
          <el-input v-model="wpSpotTradeForm.orderLocalID" placeholder="请输入本地报单编号" />
        </el-form-item>
        <el-form-item label="结算会员编号" prop="clearingPartID">
          <el-input v-model="wpSpotTradeForm.clearingPartID" placeholder="请输入结算会员编号" />
        </el-form-item>
        <el-form-item label="业务单元" prop="businessUnit">
          <el-input v-model="wpSpotTradeForm.businessUnit" placeholder="请输入业务单元" />
        </el-form-item>
        <el-form-item label="序号" prop="sequenceNo">
          <el-input v-model="wpSpotTradeForm.sequenceNo" placeholder="请输入序号" />
        </el-form-item>
        <el-form-item label="交易日" prop="tradingDay">
          <el-input v-model="wpSpotTradeForm.tradingDay" placeholder="请输入交易日" />
        </el-form-item>
        <el-form-item label="结算编号" prop="settlementID">
          <el-input v-model="wpSpotTradeForm.settlementID" placeholder="请输入结算编号" />
        </el-form-item>
        <el-form-item label="经纪公司报单编号" prop="brokerOrderSeq">
          <el-input v-model="wpSpotTradeForm.brokerOrderSeq" placeholder="请输入经纪公司报单编号" />
        </el-form-item>
        <el-form-item label="成交来源(成交来源类型数据字典)" prop="tradeSource">
          <el-input v-model="wpSpotTradeForm.tradeSource" placeholder="请输入成交来源" />
        </el-form-item>
        <el-form-item label="投资单元代码" prop="investUnitID">
          <el-input v-model="wpSpotTradeForm.investUnitID" placeholder="请输入投资单元代码" />
        </el-form-item>
        <el-form-item>
          <el-button type="success" @click="doSubmit('wpSpotTradeForm')">提交</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
  </div>
...
<script>
export default {
  name: 'WpSpotTradeName',
  data() {
    return {
      wpSpotTradeLoading: true,
      wpSpotTradeDialog: false,
      wpSpotTradeData: [],
  ...
  methods: {
    ...
    doSearch: function(data, type) {
      ...
      this.wpSpotTradeLoading = true;
      this.$http({
        url: '/wpspot/wpSpotTrade/data',
        method: 'post',
        data: Object.assign(this.pageParams, this.searchForm)
      }).then(res => {
        if (res.code === 200) {
          this.wpSpotTradeData = res.rows;
          this.pageParams.totalPage = res.totalPage;
          this.pageParams.total = res.total;
          this.wpSpotTradeLoading = false;
        } else {
    ...
    dialogAdd: function() {
      ...
      this.wpSpotTradeDialog = true;
    },
    dialogEdit: function(row, column) {
      if (this.$refs.wpSpotTradeForm) {
        this.$refs.wpSpotTradeForm.resetFields();
      }
      this.$http({
        url: '/wpspot/wpSpotTrade/findBy',
        method: 'get',
        params: {
          'id': column.id
        }
    ...
    doSubmit: function(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$http({
            url: '/wpspot/wpSpotTrade/save',
            method: 'post',
            data: this.wpSpotTradeForm
          }).then(res => {
            if (res.code === 200) {
