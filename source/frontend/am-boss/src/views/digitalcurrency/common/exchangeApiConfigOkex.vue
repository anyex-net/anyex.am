<template>
  <div class="app-container">
    ...
    <el-table v-loading="exchangeApiConfigOkexLoading" :data="exchangeApiConfigOkexData" style="width:100%;margin-bottom:20px;" border row-key="id">
    ...
    />
    <el-dialog title="用户交易所api配置管理" :visible.sync="exchangeApiConfigOkexDialog" :close-on-click-modal="false" width="600">
      <el-form ref="exchangeApiConfigOkexForm" :model="exchangeApiConfigOkexForm" :rules="exchangeApiConfigOkexRules" label-width="150px" class="exchangeApiConfigOkexForm">
        <el-form-item label="平台账户id" prop="accountId">
          <el-input v-model="exchangeApiConfigOkexForm.accountId" placeholder="请输入平台账户id" />
        </el-form-item>
        <el-form-item label="交易所" prop="exchange">
          <el-input v-model="exchangeApiConfigOkexForm.exchange" readonly="readonly" placeholder="请输入交易所" />
        </el-form-item>
        <el-form-item label="交易所账号" prop="exchangeAccount">
          <el-input v-model="exchangeApiConfigOkexForm.exchangeAccount" placeholder="请输入交易所账号" />
        </el-form-item>
        <el-form-item label="APIKEY" prop="apiKey">
          <el-input v-model="exchangeApiConfigOkexForm.apiKey" placeholder="请输入APIKEY" />
        </el-form-item>
        <el-form-item label="APISECRET" prop="apiSecret">
          <el-input v-model="exchangeApiConfigOkexForm.apiSecret" placeholder="请输入APISECRET" />
        </el-form-item>
        <el-form-item label="PASSPHRASE" prop="passPhrase">
          <el-input v-model="exchangeApiConfigOkexForm.passPhrase" placeholder="请输入PASSPHRASE" />
        </el-form-item>
        <el-form-item label="权限" prop="authorigty">
          <el-select v-model="exchangeApiConfigOkexForm.authorigty" placeholder="请选择权限" value="">
            <el-option key="readonly" label="只读" value="readonly" />
            <el-option key="trade" label="交易" value="trade" />
          </el-select>
        </el-form-item>
        <el-form-item label="ip地址" prop="ip">
          <el-input v-model="exchangeApiConfigOkexForm.ip" placeholder="请输入ip地址" />
        </el-form-item>
        <el-form-item label="现货账户ID" prop="spotAccountId">
          <el-input v-model="exchangeApiConfigOkexForm.spotAccountId" placeholder="请输入现货账户ID(火币),okex为空" />
        </el-form-item>
        <el-form-item label="备注" prop="remark">
          <el-input v-model="exchangeApiConfigOkexForm.remark" placeholder="请输入备注" />
        </el-form-item>
        <el-form-item label="关联只读api" prop="relatedExchangeAccount">
          <el-input v-model="exchangeApiConfigOkexForm.relatedExchangeAccount" placeholder="请输入关联只读api" />
        </el-form-item>
        <el-form-item>
          <el-button type="success" @click="doSubmit('exchangeApiConfigOkexForm')">提交</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
  </div>
...
<script>
export default {
  name: 'ExchangeApiConfigOkexName',
  data() {
    return {
      exchangeApiConfigOkexLoading: true,
      exchangeApiConfigOkexDialog: false,
      exchangeApiConfigOkexData: [],
      commodityTypeSelects: [],
      dicts: [],
      exchangeApiConfigOkexForm: {
        'id': '',
        'accountId': '',
        'exchange': 'OKEx',
        'exchangeAccount': '',
  ...
  methods: {
    ...
    doSearch: function(data, type) {
      ...
      this.exchangeApiConfigOkexLoading = true;
      this.$http({
        ...
        if (res.code === 200) {
          this.exchangeApiConfigOkexData = res.rows;
          this.pageParams.totalPage = res.totalPage;
          this.pageParams.total = res.total;
          this.exchangeApiConfigOkexLoading = false;
        } else {
    ...
    dialogAdd: function() {
      this.exchangeApiConfigOkexForm = {
        'id': '',
        'accountId': '',
        'exchange': 'OKEx',
        'exchangeAccount': '',
      ...
      this.exchangeApiConfigOkexDialog = true;
    },
    dialogEdit: function(row, column) {
      if (this.$refs.exchangeApiConfigOkexForm) {
        this.$refs.exchangeApiConfigOkexForm.resetFields();
      }
      this.$http({
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$http({
            ...
            method: 'post',
            data: this.exchangeApiConfigOkexForm
          }).then(res => {
            if (res.code === 200) {
          ...
          this.exchangeApiConfigOkexDialog = false;
        }
      });
    },
