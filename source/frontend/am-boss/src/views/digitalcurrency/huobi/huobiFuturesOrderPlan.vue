  ...
  <div class="app-container">
    <el-form ref="searchForm" :model="searchForm" :inline="true" size="mini">
      ...
      <el-form-item label="订单id">
        <el-input v-model="searchForm.relationOrderId" clearable placeholder="请输入订单id"></el-input>
      </el-form-item>
    ...
    <el-table v-loading="huobiFuturesOrderPlanLoading" :data="huobiFuturesOrderPlanData"
      ...
      <el-table-column prop="orderInsertAt" label="下order单时间"/>
      <el-table-column prop="canceledAt" label="撤单时间"/>
      <el-table-column prop="updateTime" label="订单更新时间"/>
    ...
    />
    <el-dialog title="交割合约计划委托管理" :visible.sync="huobiFuturesOrderPlanDialog" :close-on-click-modal="false" width="600">
      <el-form ref="huobiFuturesOrderPlanForm" :model="huobiFuturesOrderPlanForm" :rules="huobiFuturesOrderPlanRules"
               label-width="150px" class="huobiFuturesOrderPlanForm">
        <el-form-item label="主键" prop="id">
          <el-input v-model="huobiFuturesOrderPlanForm.id" placeholder="请输入主键"/>
        </el-form-item>
        <el-form-item label="平台账户ID" prop="accountId">
          <el-input v-model="huobiFuturesOrderPlanForm.accountId" placeholder="请输入平台账户ID"/>
        </el-form-item>
        <el-form-item label="外部平台apikey" prop="apiKey">
          <el-input v-model="huobiFuturesOrderPlanForm.apiKey" placeholder="请输入外部平台apikey"/>
        </el-form-item>
        <el-form-item label="合约品种" prop="symbol">
          <el-input v-model="huobiFuturesOrderPlanForm.symbol" placeholder="请输入合约品种"/>
        </el-form-item>
        <el-form-item label="合约代码" prop="contractCode">
          <el-input v-model="huobiFuturesOrderPlanForm.contractCode" placeholder="请输入合约代码"/>
        </el-form-item>
        <el-form-item label="合约类型" prop="contractType">
          <el-input v-model="huobiFuturesOrderPlanForm.contractType" placeholder="请输入合约类型"/>
        </el-form-item>
        <el-form-item label="触发类型" prop="triggerType">
          <el-input v-model="huobiFuturesOrderPlanForm.triggerType" placeholder="请输入触发类型"/>
        </el-form-item>
        <el-form-item label="委托数量" prop="volume">
          <el-input v-model="huobiFuturesOrderPlanForm.volume" placeholder="请输入委托数量"/>
        </el-form-item>
        <el-form-item label="订单类型" prop="orderType">
          <el-input v-model="huobiFuturesOrderPlanForm.orderType" placeholder="请输入订单类型"/>
        </el-form-item>
        <el-form-item label="订单方向" prop="direction">
          <el-input v-model="huobiFuturesOrderPlanForm.direction" placeholder="请输入订单方向"/>
        </el-form-item>
        <el-form-item label="开平标志" prop="offset">
          <el-input v-model="huobiFuturesOrderPlanForm.offset" placeholder="请输入开平标志"/>
        </el-form-item>
        <el-form-item label="杠杆倍数" prop="leverRate">
          <el-input v-model="huobiFuturesOrderPlanForm.leverRate" placeholder="请输入杠杆倍数"/>
        </el-form-item>
        <el-form-item label="计划委托单订单ID" prop="orderId">
          <el-input v-model="huobiFuturesOrderPlanForm.orderId" placeholder="请输入计划委托单订单ID"/>
        </el-form-item>
        <el-form-item label="字符串类型的订单ID" prop="orderIdStr">
          <el-input v-model="huobiFuturesOrderPlanForm.orderIdStr" placeholder="请输入字符串类型的订单ID"/>
        </el-form-item>
        <el-form-item label="订单id" prop="relationOrderId">
          <el-input v-model="huobiFuturesOrderPlanForm.relationOrderId" placeholder="请输入订单id"/>
        </el-form-item>
        <el-form-item label="订单报价类型" prop="orderPriceType">
          <el-input v-model="huobiFuturesOrderPlanForm.orderPriceType" placeholder="请输入订单报价类型"/>
        </el-form-item>
        <el-form-item label="订单状态" prop="status">
          <el-input v-model="huobiFuturesOrderPlanForm.status" placeholder="请输入订单状态"/>
        </el-form-item>
        <el-form-item label="来源" prop="orderSource">
          <el-input v-model="huobiFuturesOrderPlanForm.orderSource" placeholder="请输入来源"/>
        </el-form-item>
        <el-form-item label="触发价" prop="triggerPrice">
          <el-input v-model="huobiFuturesOrderPlanForm.triggerPrice" placeholder="请输入触发价"/>
        </el-form-item>
        <el-form-item label="被触发时的价格" prop="triggeredPrice">
          <el-input v-model="huobiFuturesOrderPlanForm.triggeredPrice" placeholder="请输入被触发时的价格"/>
        </el-form-item>
        <el-form-item label="委托价" prop="orderPrice">
          <el-input v-model="huobiFuturesOrderPlanForm.orderPrice" placeholder="请输入委托价"/>
        </el-form-item>
        <el-form-item label="订单创建时间" prop="createdAt">
          <el-input v-model="huobiFuturesOrderPlanForm.createdAt" placeholder="请输入订单创建时间"/>
        </el-form-item>
        <el-form-item label="触发时间" prop="triggeredAt">
          <el-input v-model="huobiFuturesOrderPlanForm.triggeredAt" placeholder="请输入触发时间"/>
        </el-form-item>
        <el-form-item label="下order单时间" prop="orderInsertAt">
          <el-input v-model="huobiFuturesOrderPlanForm.orderInsertAt" placeholder="请输入下order单时间"/>
        </el-form-item>
        <el-form-item label="撤单时间" prop="canceledAt">
          <el-input v-model="huobiFuturesOrderPlanForm.canceledAt" placeholder="请输入撤单时间"/>
        </el-form-item>
        <el-form-item label="订单更新时间" prop="updateTime">
          <el-input v-model="huobiFuturesOrderPlanForm.updateTime" placeholder="请输入订单更新时间"/>
        </el-form-item>
        <el-form-item label="被触发时下order单失败错误码" prop="failCode">
          <el-input v-model="huobiFuturesOrderPlanForm.failCode" placeholder="请输入被触发时下order单失败错误码"/>
        </el-form-item>
        <el-form-item label="被触发时下order单失败原因" prop="failReason">
          <el-input v-model="huobiFuturesOrderPlanForm.failReason" placeholder="请输入被触发时下order单失败原因"/>
        </el-form-item>
        <el-form-item>
          <el-button type="success" @click="doSubmit('huobiFuturesOrderPlanForm')">提交</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
  </div>
...
<script>
  export default {
    name: 'HuobiFuturesOrderPlanName',
    data() {
      return {
        huobiFuturesOrderPlanLoading: true,
        huobiFuturesOrderPlanDialog: false,
        huobiFuturesOrderPlanData: [],
        commodityTypeSelects: [],
        dicts: [],
        huobiFuturesOrderPlanForm: {
          'id': '',
          'accountId': '',
          'apiKey': '',
          'symbol': '',
    ...
    methods: {
      ...
      doInitData() {
        this.$http({
          url: '/digitalcurrency/huobi/dict/huobiFuturesOrderPlan',
          method: 'get'
        }).then(res => {
          if (res.code === 200) {
      ...
      doSearch: function(data, type) {
        ...
        this.huobiFuturesOrderPlanLoading = true;
        this.$http({
          url: '/digitalcurrency/huobi/huobiFuturesOrderPlan/data',
          method: 'post',
          data: Object.assign(this.pageParams, this.searchForm)
        }).then(res => {
          if (res.code === 200) {
            this.huobiFuturesOrderPlanData = res.rows;
            this.pageParams.totalPage = res.totalPage;
            this.pageParams.total = res.total;
            this.huobiFuturesOrderPlanLoading = false;
          } else {
      ...
      dialogAdd: function() {
        this.huobiFuturesOrderPlanForm = {
          'id': '',
          'accountId': '',
          'apiKey': '',
          'symbol': '',
        ...
        this.huobiFuturesOrderPlanDialog = true;
      },
      dialogEdit: function(row, column) {
        if (this.$refs.huobiFuturesOrderPlanForm) {
          this.$refs.huobiFuturesOrderPlanForm.resetFields();
        }
        this.$http({
          url: '/digitalcurrency/huobi/huobiFuturesOrderPlan/findBy',
          method: 'get',
          params: {
            'id': column.id
          }
      ...
      doSubmit: function(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.$http({
              url: '/digitalcurrency/huobi/huobiFuturesOrderPlan/save',
              method: 'post',
              data: this.huobiFuturesOrderPlanForm
            }).then(res => {
              if (res.code === 200) {
            ...
            this.huobiFuturesOrderPlanDialog = false;
          }
        });
      },
      doDelete: function(row, column) {
        this.$confirm('确认删除该记录吗, 是否继续?', '提示', {
          ...
          this.$http({
            url: '/digitalcurrency/huobi/huobiFuturesOrderPlan/del',
            method: 'post',
            data: {
              ids: column.id
            }
