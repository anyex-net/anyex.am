<template>
  <div class="app-container">
    ...
    <el-table v-loading="huobiFuturesOrderTpslLoading" :data="huobiFuturesOrderTpslData"
    ...
    />
    <el-dialog title="交割合约止盈止损委托管理" :visible.sync="huobiFuturesOrderTpslDialog" :close-on-click-modal="false"
               width="600">
      <el-form ref="huobiFuturesOrderTpslForm" :model="huobiFuturesOrderTpslForm" :rules="huobiFuturesOrderTpslRules"
               label-width="150px" class="huobiFuturesOrderTpslForm">
        <el-form-item label="主键" prop="id">
          <el-input v-model="huobiFuturesOrderTpslForm.id" placeholder="请输入主键"/>
        </el-form-item>
        <el-form-item label="平台账户ID" prop="accountId">
          <el-input v-model="huobiFuturesOrderTpslForm.accountId" placeholder="请输入平台账户ID"/>
        </el-form-item>
        <el-form-item label="外部平台apikey" prop="apiKey">
          <el-input v-model="huobiFuturesOrderTpslForm.apiKey" placeholder="请输入外部平台apikey"/>
        </el-form-item>
        <el-form-item label="品种代码" prop="symbol">
          <el-input v-model="huobiFuturesOrderTpslForm.symbol" placeholder="请输入品种代码"/>
        </el-form-item>
        <el-form-item label="合约类型" prop="contractType">
          <el-input v-model="huobiFuturesOrderTpslForm.contractType" placeholder="请输入合约类型"/>
        </el-form-item>
        <el-form-item label="合约代码" prop="contractCode">
          <el-input v-model="huobiFuturesOrderTpslForm.contractCode" placeholder="请输入合约代码"/>
        </el-form-item>
        <el-form-item label="委托数量" prop="volume">
          <el-input v-model="huobiFuturesOrderTpslForm.volume" placeholder="请输入委托数量"/>
        </el-form-item>
        <el-form-item label="订单类型" prop="orderType">
          <el-input v-model="huobiFuturesOrderTpslForm.orderType" placeholder="请输入订单类型"/>
        </el-form-item>
        <el-form-item label="止盈止损类型" prop="tpslOrderType">
          <el-input v-model="huobiFuturesOrderTpslForm.tpslOrderType" placeholder="请输入止盈止损类型"/>
        </el-form-item>
        <el-form-item label="买卖方向" prop="direction">
          <el-input v-model="huobiFuturesOrderTpslForm.direction" placeholder="请输入买卖方向"/>
        </el-form-item>
        <el-form-item label="止盈止损订单ID" prop="orderId">
          <el-input v-model="huobiFuturesOrderTpslForm.orderId" placeholder="请输入止盈止损订单ID"/>
        </el-form-item>
        <el-form-item label="字符串类型的止盈止损订单ID" prop="orderIdStr">
          <el-input v-model="huobiFuturesOrderTpslForm.orderIdStr" placeholder="请输入字符串类型的止盈止损订单ID"/>
        </el-form-item>
        <el-form-item label="来源" prop="orderSource">
          <el-input v-model="huobiFuturesOrderTpslForm.orderSource" placeholder="请输入来源"/>
        </el-form-item>
        <el-form-item label="委托价" prop="orderPrice">
          <el-input v-model="huobiFuturesOrderTpslForm.orderPrice" placeholder="请输入委托价"/>
        </el-form-item>
        <el-form-item label="触发类型" prop="triggerType">
          <el-input v-model="huobiFuturesOrderTpslForm.triggerType" placeholder="请输入触发类型"/>
        </el-form-item>
        <el-form-item label="触发价" prop="triggerPrice">
          <el-input v-model="huobiFuturesOrderTpslForm.triggerPrice" placeholder="请输入触发价"/>
        </el-form-item>
        <el-form-item label="订单创建时间" prop="createdAt">
          <el-input v-model="huobiFuturesOrderTpslForm.createdAt" placeholder="请输入订单创建时间"/>
        </el-form-item>
        <el-form-item label="订单报价类型" prop="orderPriceType">
          <el-input v-model="huobiFuturesOrderTpslForm.orderPriceType" placeholder="请输入订单报价类型"/>
        </el-form-item>
        <el-form-item label="订单状态：" prop="status">
          <el-input v-model="huobiFuturesOrderTpslForm.status" placeholder="请输入订单状态："/>
        </el-form-item>
        <el-form-item label="源限价单的订单id" prop="sourceOrderId">
          <el-input v-model="huobiFuturesOrderTpslForm.sourceOrderId" placeholder="请输入源限价单的订单id"/>
        </el-form-item>
        <el-form-item label="关联的止盈止损单id" prop="relationTpslOrderId">
          <el-input v-model="huobiFuturesOrderTpslForm.relationTpslOrderId" placeholder="请输入关联的止盈止损单id"/>
        </el-form-item>
        <el-form-item>
          <el-button type="success" @click="doSubmit('huobiFuturesOrderTpslForm')">提交</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
  </div>
...
<script>
  export default {
    name: 'HuobiFuturesOrderTpslName',
    data() {
      return {
        huobiFuturesOrderTpslLoading: true,
        huobiFuturesOrderTpslDialog: false,
        huobiFuturesOrderTpslData: [],
        commodityTypeSelects: [],
        dicts: [],
        huobiFuturesOrderTpslForm: {
          'id': '',
          'accountId': '',
          'apiKey': '',
          'symbol': '',
    ...
    methods: {
      ...
      doInitData() {
        this.$http({
          url: '/digitalcurrency/huobi/dict/huobiFuturesOrderTpsl',
          method: 'get'
        }).then(res => {
          if (res.code === 200) {
      ...
      doSearch: function(data, type) {
        ...
        this.huobiFuturesOrderTpslLoading = true;
        this.$http({
          url: '/digitalcurrency/huobi/huobiFuturesOrderTpsl/data',
          method: 'post',
          data: Object.assign(this.pageParams, this.searchForm)
        }).then(res => {
          if (res.code === 200) {
            this.huobiFuturesOrderTpslData = res.rows;
            this.pageParams.totalPage = res.totalPage;
            this.pageParams.total = res.total;
            this.huobiFuturesOrderTpslLoading = false;
          } else {
      ...
      dialogAdd: function() {
        this.huobiFuturesOrderTpslForm = {
          'id': '',
          'accountId': '',
          'apiKey': '',
          'symbol': '',
        ...
        this.huobiFuturesOrderTpslDialog = true;
      },
      dialogEdit: function(row, column) {
        if (this.$refs.huobiFuturesOrderTpslForm) {
          this.$refs.huobiFuturesOrderTpslForm.resetFields();
        }
        this.$http({
          url: '/digitalcurrency/huobi/huobiFuturesOrderTpsl/findBy',
          method: 'get',
          params: {
            'id': column.id
          }
      ...
      doSubmit: function(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.$http({
              url: '/digitalcurrency/huobi/huobiFuturesOrderTpsl/save',
              method: 'post',
              data: this.huobiFuturesOrderTpslForm
            }).then(res => {
              if (res.code === 200) {
            ...
            this.huobiFuturesOrderTpslDialog = false;
          }
        });
      },
      doDelete: function(row, column) {
        this.$confirm('确认删除该记录吗, 是否继续?', '提示', {
          ...
          this.$http({
            url: '/digitalcurrency/huobi/huobiFuturesOrderTpsl/del',
            method: 'post',
            data: {
              ids: column.id
            }
