...
import 'package:amclient_flutter/WPSpot/Model/WPSpotAccountInfoResponseModel.dart';
...
class WPSpotAccountInfoView extends StatefulWidget {
  final arguments;
  const WPSpotAccountInfoView({Key? key, this.arguments}) : super(key: key);

  @override
  _WPSpotAccountInfoViewState createState() => _WPSpotAccountInfoViewState();
}

class _WPSpotAccountInfoViewState extends State<WPSpotAccountInfoView> {


  late List<WPSpotAccountInfoResponseData> _accountInfoList = [];

  @override
  void initState() {
    super.initState();
    _WpSpotAccountInfoAccountInfoDataHttp();

    neverBus.on('WPSpotAccountInfoUpdate', (object) {
      if(mounted){
        _WpSpotAccountInfoAccountInfoDataHttp();
      }
    });
  ...
  @override
  void dispose() {
    neverBus.off("WPSpotAccountInfoUpdate");
    super.dispose();
  }

  //外盘现货 查询外盘现货资金账号
  Future _WpSpotAccountInfoAccountInfoDataHttp() async{
    var response = await WpSpotAccountInfoAccountInfoData();
    if (response is Map) {
      if(!mounted){
        return;
      }
      if(response['code'] == 200){
        setState(() {
          _accountInfoList = WPSpotAccountInfoResponseModel.fromJson(response).object!;
        });
      }else {
        if(response.containsKey('message')){
          showMyCustomText(response["message"]);
        }else{
          showMyCustomText('[查询外盘现货资金账号]遇到问题，请检查网络或重新刷新');
        }
      }
    }else {
      try{
          ...
          GestureDetector(
            child: Container(
              child: Container(
                ...
                child: const Text('新增外盘现货API',style: TextStyle(color: Colors.white,fontSize: 12),softWrap: true,overflow: TextOverflow.ellipsis,maxLines: 1,),
            ...
            onTap: (){
              ...
              AwesomeDialog(
                ...
                body:const  WPSpotAccountInfoAddView(),
              ).show();
            },
          ),
                        ...
                        child: IntrinsicHeight(
                          child: Row(
                            children: [
                              ...
                              Expanded(
                                child: Container(
                                  child: Text(_accountInfoList[index].mdFrontAddr!,style: const TextStyle(fontSize: 11,color: Colors.white),),
                                  alignment: Alignment.center,
                                ),
                                flex: 3,
                              ),
                              ...
                              Expanded(
                                child: Container(
                                  child: Text(_accountInfoList[index].tradeFrontAddr!,style: const TextStyle(fontSize: 11,color: Colors.white),),
                                  alignment: Alignment.center,
                                ),
                                flex: 3,
                              ),
                              ...
                              Expanded(
                                child:Container(
                                  child: GestureDetector(
                                    ...
                                    onTap: () async{
                                      FocusScope.of(context).requestFocus(FocusNode());
                                      var response = await WpSpotAccountInfoAccountInfoDel(id: _accountInfoList[index].id!);
                                      if (response is Map) {
                                        if(!mounted){
                                          return;
                                        }
                                        if(response['code'] == 200){
                                          ...
                                          neverBus.emit('WPSpotAccountInfoUpdate');
                                        }else {
                                          if(response.containsKey('message')){
                                            showMyCustomText(response["message"]);
                                          }else{
