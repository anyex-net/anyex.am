...
import 'package:amclient_flutter/WPSpot/Model/WPSpotBrokerInfoResponseModel.dart';
import 'package:flutter/cupertino.dart';
import 'package:flutter/material.dart';
import 'package:neveruseless/neveruseless.dart';
class WPSpotAccountInfoAddView extends StatefulWidget {
  final arguments;
  const WPSpotAccountInfoAddView({Key? key, this.arguments}) : super(key: key);

  @override
  _WPSpotAccountInfoAddViewState createState() => _WPSpotAccountInfoAddViewState();
}

class _WPSpotAccountInfoAddViewState extends State<WPSpotAccountInfoAddView> {

  late List<WPSpotBrokerInfoResponseData> _brokerInfoList = [];
  ...
  //外盘现货 新增API 经销商编码
  Future _WpSpotBrokerInfoDataHttp() async{
    var response = await WpSpotBrokerInfoData();
    if (response is Map) {
      if(!mounted){
        return;
      }
      if(response['code'] == 200){
        setState(() {
          _brokerInfoList = WPSpotBrokerInfoResponseModel.fromJson(response).object!;
        ...
        if(response.containsKey('message')){
          ...
          showMyCustomText('[查询外盘现货经纪公司]遇到问题，请检查网络或重新刷新');
        }
      }
    }else {
      try{
  ...
  //外盘现货 新增API 新增
  Future _WpSpotAccountInfoAccountInfoSaveHttp() async{//{String accountAuthCode = '',String accountNo = '',String accountPassword = '',String brokerNo = '',}
    var response = await WpSpotAccountInfoAccountInfoSave(
    ...
    if (response is Map) {
      ...
      if(response['code'] == 200){
        ...
        if(response.containsKey('message')){
          ...
          showMyCustomText('[新增外盘现货API]遇到问题，请检查网络或重新刷新');
        }
      }
    }else {
      try{
                ...
                children: [
                  Row(
                    children: [
                      Expanded(
                        child: Container(
                          child: const Text('新增外盘现货API',style: TextStyle(fontWeight: FontWeight.w500,fontSize: 16),),
                          alignment: Alignment.centerLeft,
                        ),
                      ),
                      GestureDetector(
                      ...
                      children: [
                        ...
                        GestureDetector(
                          ...
                          onTap: (){
                            ...
                            _WpSpotAccountInfoAccountInfoSaveHttp();
                          },
                        ),
                      ],
                    ),
