...
class NPFuturesAccountInfoView extends StatefulWidget {
  final arguments;
  const NPFuturesAccountInfoView({Key? key, this.arguments}) : super(key: key);

  @override
  _NPFuturesAccountInfoViewState createState() => _NPFuturesAccountInfoViewState();
}

class _NPFuturesAccountInfoViewState extends State<NPFuturesAccountInfoView> {
  ...
  void initState() {
    super.initState();
    _NpfuturesNpfuturesAccountInfoDataHttp();

    neverBus.on('NPFuturesAccountInfoUpdate', (object) {
      if(mounted){
        _NpfuturesNpfuturesAccountInfoDataHttp();
      }
    });
  ...
  @override
  void dispose() {
    neverBus.off("NPFuturesAccountInfoUpdate");
    super.dispose();
  }

  //内盘期货 查询外盘期货资金账号
  Future _NpfuturesNpfuturesAccountInfoDataHttp() async{
    var response = await NpfuturesNpfuturesAccountInfoData();
    if (response is Map) {
      if(!mounted){
        return;
      }
            ...
            child: GestureDetector(
              child: Container(
                child: Container(
                  ...
                  child: const Text('新增内盘期货API',style: TextStyle(color: Colors.white,fontSize: 12),softWrap: true,overflow: TextOverflow.ellipsis,maxLines: 1,),
              ...
              onTap: (){
                ...
                AwesomeDialog(
                  ...
                  body: const NPFuturesAccountInfoAddView(),
                ).show();
              },
            ),
          ),
                              ...
                              Expanded(
                                child: Container(
                                  child: GestureDetector(
                                    ...
                                    onTap: () async{
                                      FocusScope.of(context).requestFocus(FocusNode());
                                      var response = await NPFuturesNpfuturesAccountInfoDel(id: _accountInfoList[index].id!);
                                      if (response is Map) {
                                        if(!mounted){
                                          return;
                                        }
