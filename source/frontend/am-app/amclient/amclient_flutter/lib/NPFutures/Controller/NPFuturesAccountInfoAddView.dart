...
class NPFuturesAccountInfoAddView extends StatefulWidget {
  final arguments;
  const NPFuturesAccountInfoAddView({Key? key, this.arguments}) : super(key: key);

  @override
  _NPFuturesAccountInfoAddViewState createState() => _NPFuturesAccountInfoAddViewState();
}

class _NPFuturesAccountInfoAddViewState extends State<NPFuturesAccountInfoAddView> {
  ...
  void initState() {
    super.initState();
    _NPFuturesWpfuturesBrokerInfoDataHttp();
  }
  ...
  //内盘期货 新增API 经销商编码
  Future _NPFuturesWpfuturesBrokerInfoDataHttp() async{
    var response = await NpfuturesNpfuturesBrokerData();
    if (response is Map) {
      if(!mounted){
        return;
      }
  ...
  //内盘期货 新增API 新增
  Future _NpfuturesNpfuturesAccountInfoSaveHttp() async{//{String accountAuthCode = '',String accountNo = '',String accountPassword = '',String brokerNo = '',}
    var response = await NpfuturesNpfuturesAccountInfoSave(
    ...
    if (response is Map) {
      ...
      if(response['code'] == 200){
        ...
        if(response.containsKey('message')){
          ...
          showMyCustomText('[新增内盘期货API]遇到问题，请检查网络或重新刷新');
        }
      }
    }else {
      try{
                ...
                children: [
                  Row(
                    children: [
                      Expanded(
                        child: Container(
                          child: const Text('新增内盘期货API',style: TextStyle(fontWeight: FontWeight.w500,fontSize: 16),),
                          alignment: Alignment.centerLeft,
                        ),
                      ),
                      GestureDetector(
                      ...
                      children: [
                        ...
                        GestureDetector(
                          ...
                          onTap: (){
                            ...
                            _NpfuturesNpfuturesAccountInfoSaveHttp();
                          },
                        ),
                      ],
                    ),
