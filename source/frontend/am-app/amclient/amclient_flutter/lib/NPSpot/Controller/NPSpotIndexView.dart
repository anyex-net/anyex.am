...
class NPSpotIndexView extends StatefulWidget {
  final arguments;
  const NPSpotIndexView({Key? key, this.arguments}) : super(key: key);

  @override
  _NPSpotIndexViewState createState() => _NPSpotIndexViewState();
}
class _NPSpotIndexViewState extends State<NPSpotIndexView> with AutomaticKeepAliveClientMixin,SingleTickerProviderStateMixin {

  //币对选择页面是否显示
  bool _isShowCurrencyPick = true;
  //外盘期货交易所信息
  List<NpFuturesExchangeInfoListData>? _npSpotExchangeList = []; //_npFutures ExchangeList _npSpotExchangeList
  //选择的交易所
  ...
  //内盘期货交易所现货证券信息管理
  List<NpFuturesExchangeInstrumentListData>? _npSpotExchangeInstrumentList = []; // _npFutures ExchangeInstrumentList _npSpotExchangeInstrumentList
  List<NpFuturesExchangeInstrumentListData>? _tempNpSpotExchangeInstrumentList = [];//_tempNpFutures ExchangeInstrumentList _tempNpSpotExchangeInstrumentList
  //选择的内盘期货交易所期货证券ID
  ...
  List<NpFuturesExchangeInstrumentListData>? _screenList = [];//筛选
  final TextEditingController _currencyController = TextEditingController();

  //内盘现货资金账号资金信息
  List<NpfuturesTradingAccountListData>? _npSpotTradingAccountList = []; // _npFutures TradingAccountList

  //内盘现货成交
  List<NpfuturesTradeListData>? _npSpotTradeList = []; //_npFutures TradeList  _npSpotTradeList
  int _npSpotTradePage = 1; // _npFutures TradePage _npSpotTradePage
  bool? _npSpotTradehasNextPage = false; //_npFutures TradehasNextPage _npSpotTradehasNextPage

  //内盘现货所有持仓信息
  List<NpfuturesInvestorPositionListData>? _npSpotInvestorPositionList = []; // _npFutures InvestorPositionList _npSpotInvestorPositionList
  int _npSpotInvestorPositionPage = 1; // _npFutures InvestorPositionPage _npSpotInvestorPositionPage
  bool? _npSpotInvestorPositionhasNextPage = false; // _npFutures InvestorPositionhasNextPage _npSpotInvestorPositionhasNextPage

  //内盘现货委托订单
  List<NpfuturesOrderListData>? _npSpotOrderList= []; //  _npFutures OrderList _npSpotOrderList
  int _npSpotOrderInfoPage = 1; // _npSpotOrderInfoPage
  bool? _npSpotOrderInfohasNextPage = false; // _npFuturesOrder InfohasNextPage  _npSpotOrderInfohasNextPage

  //内盘现货历史委托订单
  List<NpfuturesOrderListData>? _npSpotOrderHisList= []; //_npFutures OrderHisList _npSpotOrderHisList
  int _npSpotOrderInfoHisPage = 1; //_npFutures OrderInfoHisPage _npSpotOrderInfoHisPage
  ...
  void initState() {
    ...
    _NpSpotExchangeQryCtpCommonDataHttp();
    ...
    neverBus.on('LoginSuccess', (object){
      ...
      _NpSpotExchangeQryCtpCommonDataHttp();
      _NpSpotExchangeInstrumentInitHttp();
    });
    neverBus.on('NPFuturesIndexReload', (object){
      if(!mounted){
        return;
      }
      _NpSpotExchangeDataHttp();
  ...
  //初始化内盘现货部分公共数据
  Future _NpSpotExchangeQryCtpCommonDataHttp() async{
    var response = await NpSpotExchangeQryCtpCommonData();
    if (response is Map) {
      if(!mounted){
        return;
      }
  ...
  //内盘现货交易所信息
  Future _NpSpotExchangeDataHttp() async{
    var response = await NpSpotExchangeData();
    if (response is Map) {
      if(!mounted){
        return;
      }
      if(response['code'] == 200){
        setState(() {
          _npSpotExchangeList = NpFuturesExchangeInfoResponseData.fromJson(response).object;
        });
      }else {
        try{
          showMyCustomText(response['message']);
        }catch(e){
          showMyCustomText('[查询内盘现货交易所]遇到问题，请检查网络或重新刷新');
        }
      }
    }else {
      try{
  ...
  //内盘现货交易所现货证劵表
  Future _NpSpotExchangeInstrumentDataHttp({String exchangeID = '',String instrumentID = '',String productID = ''}) async{
    var response = await NpSpotExchangeInstrumentData(exchangeID: exchangeID,instrumentID:instrumentID,productID:productID);
    if (response is Map) {
      if(!mounted){
        return;
      }
      if(response['code'] == 200){
        setState(() {
          _npSpotExchangeInstrumentList = NpFuturesExchangeInstrumentResponseData.fromJson(response).object;
        ...
        try{
          ...
          showMyCustomText('[查询内盘现货证券]遇到问题，请检查网络或重新刷新');
        }
      }
    }else {
      try{
  ...
  //内盘现货交易所现货证劵 -默认款
  Future _NpSpotExchangeInstrumentInitHttp() async{
    var response = await NpSpotExchangeInstrumentInit();
    if (response is Map) {
      if(!mounted){
        return;
      }
      if(response['code'] == 200){
        ...
        setState(() {
          ...
          _NpSpotExchangeInstrumentDataHttp(exchangeID: _selectIndexExchangeNo!,);
          _onSendSocket();
        });
      }else {
        try{
    ...
    if (response is Map) {
      ...
      if(response['code'] == 200){
        ...
        try{
          ...
          showMyCustomText('[查询内盘现货资金账户]遇到问题，请检查网络或重新刷新');
        }
      }
    }else {
      try{
  ...
  //查询内盘现货委托订单表
  Future _NpSpotOrderDataHttp() async{
    var response = await NpSpotOrderData(page: _npSpotOrderInfoPage);
    if (response is Map) {
      if(!mounted){
        return;
      }
      if(response['code'] == 200){
        setState(() {
          if (_npSpotOrderInfoPage == 1) {
            _npSpotOrderList = NpfuturesOrderResponseData.fromJson(response).rows;
          } else {
            _npSpotOrderList!.addAll(NpfuturesOrderResponseData.fromJson(response).rows!.toList());
          }
          _npSpotOrderInfohasNextPage = NpfuturesOrderResponseData.fromJson(response).hasNext ?? false;
        });
      }else {
        try{
          showMyCustomText(response['message']);
        }catch(e){
          showMyCustomText('[查询内盘现货委托订单]遇到问题，请检查网络或重新刷新');
        }
      }
    }else {
      try{
  ...
  //查询内盘现货历史委托订单表
  Future _NpSpotOrderHisDataHttp() async{
    var response = await NpSpotOrderHisData(page: _npSpotOrderInfoHisPage);
    if (response is Map) {
      if(!mounted){
        return;
      }
      if(response['code'] == 200){
        setState(() {
          if (_npSpotOrderInfoHisPage == 1) {
            _npSpotOrderHisList = NpfuturesOrderResponseData.fromJson(response).rows;
          } else {
            _npSpotOrderHisList!.addAll(NpfuturesOrderResponseData.fromJson(response).rows!.toList());
          }
          _npSpotOrderInfoHishasNextPage = NpfuturesOrderResponseData.fromJson(response).hasNext ?? false;
        });
      }else {
        try{
          showMyCustomText(response['message']);
        }catch(e){
          showMyCustomText('[查询内盘现货历史委托]遇到问题，请检查网络或重新刷新');
        }
      }
    }else {
      try{
  ...
  //查询内盘现货成交表
  Future _NpSpotTradeDataHttp() async{
    var response = await NpSpotTradeData(page: _npSpotTradePage);
    if (response is Map) {
      if(!mounted){
        return;
      }
      if(response['code'] == 200){
        setState(() {
          if (_npSpotTradePage == 1) {
            _npSpotTradeList = NpfuturesTradeResponseData.fromJson(response).rows;
          } else {
            _npSpotTradeList!.addAll(NpfuturesTradeResponseData.fromJson(response).rows!.toList());
          }
          _npSpotTradehasNextPage = NpfuturesTradeResponseData.fromJson(response).hasNext ?? false;
        });
      }else {
        try{
          showMyCustomText(response['message']);
        }catch(e){
          showMyCustomText('[查询内盘现货成交]遇到问题，请检查网络或重新刷新');
        }
      }
    }else {
      try{
  ...
  //查询内盘现货持仓汇总表
  Future _NpSpotInvestorPositionDataHttp() async{
    var response = await NpSpotInvestorPositionData(page: _npSpotInvestorPositionPage);
    if (response is Map) {
      if(!mounted){
        return;
      }
      if(response['code'] == 200){
        setState(() {
          if (_npSpotInvestorPositionPage == 1) {
            _npSpotInvestorPositionList = NpfuturesInvestorPositionResponseData.fromJson(response).rows;
          } else {
            _npSpotInvestorPositionList!.addAll(NpfuturesInvestorPositionResponseData.fromJson(response).rows!.toList());
          }
          _npSpotInvestorPositionhasNextPage = NpfuturesInvestorPositionResponseData.fromJson(response).hasNext ?? false;
        });
      }else {
        try{
          showMyCustomText(response['message']);
        }catch(e){
          showMyCustomText('[查询内盘现货持仓]遇到问题，请检查网络或重新刷新');
        }
      }
    }else {
      try{
  ...
  //内盘现货委托批量撤单
  Future _NpSpotOrderBatchCancelOrderHttp() async{
    var response = await NpSpotOrderBatchCancelOrder();
    if (response is Map) {
      if(!mounted){
        return;
      }
      if (response["code"] == 200) {
        ...
        Future.delayed(const Duration(milliseconds: 300)).then((onValue) async{
          _npSpotOrderInfoPage = 1;
          _NpSpotOrderDataHttp();
        });
      } else {
        try{
      ...
      try{
        ...
        showMyCustomText('[内盘现货批量撤单]遇到问题，请检查网络或重新刷新');
      }
    }
  }

  //内盘现货委托撤单
  Future _NpSpotOrderOrderActionHttp({String exchangeID = '',String instrumentID = '',String investorID = '',String orderRef = '',String frontID = '',String sessionID = ''}) async{
    var response = await NpSpotOrderOrderAction(
    ...
    if (response is Map) {
      ...
      try{
        ...
        showMyCustomText('[内盘现货委托撤单]遇到问题，请检查网络或重新刷新');
      }
    }
  }

  //内盘现货委托下单
  Future _NpSpotOrderOrderInsertHttp({String combHedgeFlag = '',String combOffsetFlag = '',String direction = '', String exchangeID = '',String instrumentID = '',String investorID = '', String limitPrice = '',String orderPriceType = '',String volumeTotalOriginal = '',}) async{
    var response = await NpSpotOrderOrderInsert(
  ...
  //内盘现货 订阅内盘现货深度行情
  Future _NpSpotDepthMarketDataSubscribeMarketDataHttp({String ppInstrumentID = ''}) async{
    // print(ppInstrumentID);
    var response = await NpSpotDepthMarketDataSubscribeMarketData(ppInstrumentID: ppInstrumentID);
    // print(response);
    if (response is Map) {
      if(!mounted){
        return;
      ...
      if(response['code'] == 200){
        ...
        try{
          ...
          showMyCustomText('[订阅内盘现货深度行情]遇到问题，请检查网络或重新刷新');
        }
      }
    }else {
      try{
  ...
  //内盘现货 取消订阅内盘现货深度行情
  Future _NpSpotDepthMarketDataUnSubscribeMarketDataHttp({String ppInstrumentID = ''}) async{
    // print(ppInstrumentID);
    var response = await NpSpotDepthMarketDataUnSubscribeMarketData(ppInstrumentID: ppInstrumentID);
    // print(response);
    if (response is Map) {
      if(!mounted){
        return;
      ...
      if(response['code'] == 200){
        ...
        try{
          ...
          showMyCustomText('[取消订阅内盘现货深度行情]遇到问题，请检查网络或重新刷新');
        }
      }
    }else {
      try{
  ...
  _onSendSocket(){
    ...
    String subsStr= "{\"subs\":\"quote.npspot.ctp2stp." + _selectIndexInstrumentID! + ".entrust,"
        + "quote.npspot.ctp2stp." + _selectIndexInstrumentID! + ".trade,"
        + "quote.npspot.ctp2stp." + _selectIndexInstrumentID! + ".kline." + _selectKlineTimeType
        + "\"}";
  ...
  _onSendUnSocket(){
    ...
    String subsStr= "{\"unsubs\":\"quote.npspot.ctp2stp." + _selectIndexInstrumentID! + ".entrust,"
        + "quote.npspot.ctp2stp." + _selectIndexInstrumentID! + ".trade,"
        + "quote.npspot.ctp2stp." + _selectIndexInstrumentID! + ".kline." + _selectKlineTimeType
        + "\"}";
  ...
  _onData(event){
    ...
    try{
      ...
      if(_initDateMap["trades"].toString() != '' && _initDateMap["trades"].toString() != 'null'){
        if(_initDateMap['topic'] == ("quote.npspot.ctp2stp." + _selectIndexInstrumentID!)){
            ...
            Container(
              child:ListView.builder(
                ...
                itemBuilder: (context,index){
                  return _selectIndexItemView(_npSpotExchangeList![index]);
                },
                itemCount: _npSpotExchangeList?.length,
              ),
              height: 50,
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context,index){
                    if(_currencyController.text.isNotEmpty){
                      ...
                      return _instrumentListCell(_npSpotExchangeInstrumentList![index]);
                    }
                  },
                  itemCount: _currencyController.text.isNotEmpty ? _screenList!.length : _npSpotExchangeInstrumentList!.length,
                ),
              ),
            ),
          ],
  ...
  Widget _instrumentListCell(NpFuturesExchangeInstrumentListData data){
    return GestureDetector(
      child: Container(
        child: Row(
          children: [
      ...
      onTap: (){
        setState(() {
          ...
          if(_selectIndexExchangeNo != "" && _selectIndexExchangeProductNo != "" && _selectIndexInstrumentID != ""){
            _NpSpotDepthMarketDataUnSubscribeMarketDataHttp(ppInstrumentID: _selectIndexInstrumentID!);
            if(_isSocektOn){
              _onSendUnSocket();
            }
          ...
          _selectIndexExchangeProductNo = data.productID!;
          ...
          _tempNpSpotExchangeInstrumentList!.addAll(_npSpotExchangeInstrumentList!);

          _selectIndexExchangeInstrumentData= data;

          if(_selectIndexExchangeNo != "" && _selectIndexExchangeProductNo != "" && _selectIndexInstrumentID != ""){
            _NpSpotDepthMarketDataSubscribeMarketDataHttp(ppInstrumentID: _selectIndexInstrumentID!);
            if(_isSocektOn){
              _onSendSocket();
            }else{
              _onRest();
  ...
  //证券类型选择按钮组件
  Widget _selectIndexItemView(NpFuturesExchangeInfoListData data){
    return GestureDetector(
      child: Container(
        alignment: Alignment.center,
        child: Column(
      ...
      onTap: (){
        ...
        setState(() {
          ...
          _npSpotExchangeInstrumentList!.clear();
          _NpSpotExchangeInstrumentDataHttp(exchangeID: _tempExchangeNo,);
        });
      },
    );
  }
  ...
  void _sceenListFuture(String value){
    try{
      _screenList = _npSpotExchangeInstrumentList!.where((e) => e.instrumentID!.contains(value)).toList();
      if(_screenList!.isEmpty){
        _screenList = _npSpotExchangeInstrumentList!.where((e) => e.instrumentName!.contains(value)).toList();
      }else{
        _screenList!.addAll(_npSpotExchangeInstrumentList!.where((e) => e.instrumentName!.contains(value)).toList());
      }
    }catch(a){
      _screenList = _npSpotExchangeInstrumentList!.where((e) => e.instrumentID!.contains(value)).toList();
      if(_screenList!.isEmpty){
    ...
    // if(double.tryParse(value) != null){ //数字
    //   _screenList = _npSpotExchangeInstrumentList!.where((e) => e.instrumentID!.contains(value)).toList();
    // }else{
    //   _screenList = _npSpotExchangeInstrumentList!.where((e) => e.instrumentName!.contains(value)).toList();
    // }
              ...
              onTap: (){
                ...
                setState(() {
                  ...
                  if(_isShowCurrencyPick == false){
                    ...
                    _npSpotExchangeInstrumentList!.clear();
                    _npSpotExchangeInstrumentList!.addAll(_tempNpSpotExchangeInstrumentList!);
                    _NpSpotExchangeDataHttp();
                    _NpSpotExchangeInstrumentDataHttp(exchangeID: _tempExchangeNo);
                  }
                });
                                ...
                                onTap: (){
                                  setState(() {
                                    ...
                                    if(_isSocektOn){
                                      ...
                                      String subsStr= "{\"unsubs\":\"quote.npspot.ctp2stp." + _selectIndexInstrumentID! + ".kline." + _selectKlineTimeType
                                          + "\"}";
                                      // print(subsStr);
                                      _channel.sink.add(subsStr);
                                    }
                                    ...
                                    if(_isSocektOn){
                                      ...
                                      String subsStr= "{\"subs\":\"quote.npspot.ctp2stp." + _selectIndexInstrumentID! + ".kline." + _selectKlineTimeType
                                          + "\"}";
                                      // print(subsStr);
                                      _channel.sink.add(subsStr);
                                    }else{
  ...
  Widget _TransactionPanelWidget(){
    ...
    if(_npSpotTradingAccountList != null && _npSpotTradingAccountList!.isNotEmpty){
      _equity = _npSpotTradingAccountList!.first.balance!;
      _available = _npSpotTradingAccountList!.first.available!;
      _balanceToPreBalance = (NumUtil.getNumByValueDouble(neverDoubleTryOrZero(_npSpotTradingAccountList!.first.balance!) - neverDoubleTryOrZero(_npSpotTradingAccountList!.first.preBalance!), 2)).toString();
      if(neverDoubleTryOrZero(_npSpotTradingAccountList!.first.preBalance!) == 0.0){
        _balanceXpreBalance = '0.0%';
      }else{
        _balanceXpreBalance = (NumUtil.getNumByValueDouble((neverDoubleTryOrZero(_npSpotTradingAccountList!.first.balance!) - neverDoubleTryOrZero(_npSpotTradingAccountList!.first.preBalance!)) / neverDoubleTryOrZero(_npSpotTradingAccountList!.first.preBalance!) * 100, 2)).toString() + '%';
      }
    }else{
      _equity = '-';
      _available = '-';
                    ...
                    Expanded(
                      child: Container(
                        child: GestureDetector(
                          ...
                          behavior: HitTestBehavior.opaque,
                          onTap: (){
                            _NpSpotOrderBatchCancelOrderHttp();
                          },
                        ),
                        alignment: Alignment.centerRight,
              ...
              _selectAssetEntrustmentType == '当前委托' ? Container(
                ...
                child:EasyRefresh(
                  ...
                  child: ListView.builder(
                      ...
                      return Container(
                        ...
                        child: Row(
                          children: [
                            Expanded(
                              child: Container(
                                child: Text(_npSpotOrderList![index].exchangeID!,style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(10, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                            ...
                            Expanded(
                              child: Container(
                                child: Text(_npSpotOrderList![index].instrumentID!,style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(0, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                            ...
                            Expanded(
                              child: Container(
                                child: Text(_npSpotOrderList![index].orderLocalID!.trim(),style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(0, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                            ...
                            Expanded(
                              child: Container(
                                child: Text(_findTrans(_npFuturesTranslate!, "ComboOffsetFlag", _npSpotOrderList![index].combOffsetFlag!),style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(0, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                            ...
                            Expanded(
                              child: Container(
                                child: Text(_findTrans(_npFuturesTranslate!, "Direction", _npSpotOrderList![index].direction!),style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(0, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                            ...
                            Expanded(
                              child: Container(
                                child: Text(_npSpotOrderList![index].limitPrice!,style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(0, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                            ...
                            Expanded(
                              child: Container(
                                child: Text(_npSpotOrderList![index].volumeTotalOriginal!,style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding:const EdgeInsets.fromLTRB(0, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                            ...
                            Expanded(
                              child: Container(
                                child: Text(_findTrans(_npFuturesTranslate!, "OrderStatus", _npSpotOrderList![index].orderStatus!),style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(0, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                            ...
                            Expanded(
                              child: Container(
                                child: Text(_npSpotOrderList![index].insertDate! + ' ' + _npSpotOrderList![index].insertTime!,style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(0, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                            ...
                            Expanded(
                              child: Container(
                                child: Text(_npSpotOrderList![index].statusMsg!,style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(0, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                              ...
                              child: Container(
                                child: Offstage(
                                  child: GestureDetector(
                                    ...
                                    behavior: HitTestBehavior.opaque,
                                    onTap: (){
                                      _NpSpotOrderOrderActionHttp(
                                        exchangeID: _npSpotOrderList![index].exchangeID!,
                                        instrumentID:_npSpotOrderList![index].instrumentID!,
                                        investorID:_npSpotOrderList![index].investorID!,
                                        orderRef:_npSpotOrderList![index].orderRef!,
                                        frontID: _npSpotOrderList![index].frontID!,
                                        sessionID: _npSpotOrderList![index].sessionID!,
                                      );
                                    },
                                  ),
                                  offstage: (_npSpotOrderList![index].orderStatus! == "1" || _npSpotOrderList![index].orderStatus! == "3") ? false : true,
                                ),
                                alignment: Alignment.centerRight,
                                padding: const EdgeInsets.fromLTRB(0, 5, 10, 5),
                              ),
                    ...
                    itemCount: _npSpotOrderList!.length,
                  ),
                  header: ClassicalHeader(
                    bgColor: Colors.transparent,
                    textColor: Colors.grey,
                  ...
                  footer: _npSpotOrderInfohasNextPage!
                      ? ClassicalFooter(
                    bgColor: Colors.transparent,
                    textColor: Colors.grey,
                    loadText: '上拉刷新',
                  ...
                  onLoad: _npSpotOrderInfohasNextPage!
                      ? () async {
                    await Future.delayed(const Duration(milliseconds: 500), () {
                      if (mounted) {
                        _npSpotOrderInfoPage += 1;
              ...
              _selectAssetEntrustmentType == '历史委托' ? Container(
                ...
                child:EasyRefresh(
                  ...
                  child: ListView.builder(
                      ...
                      return Container(
                        ...
                        child: Row(
                          children: [
                            Expanded(
                              child: Container(
                                child: Text(_npSpotOrderHisList![index].exchangeID!,style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(10, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                            ...
                            Expanded(
                              child: Container(
                                child: Text(_npSpotOrderHisList![index].instrumentID!,style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(0, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                            ...
                            Expanded(
                              child: Container(
                                child: Text(_npSpotOrderHisList![index].orderLocalID!.trim(),style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(0, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                            ...
                            Expanded(
                              child: Container(
                                child: Text(_findTrans(_npFuturesTranslate!, "ComboOffsetFlag", _npSpotOrderHisList![index].combOffsetFlag!),style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(0, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                            ...
                            Expanded(
                              child: Container(
                                child: Text(_findTrans(_npFuturesTranslate!, "Direction", _npSpotOrderHisList![index].direction!),style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(0, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                            ...
                            Expanded(
                              child: Container(
                                child: Text(_npSpotOrderHisList![index].limitPrice!,style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(0, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                            ...
                            Expanded(
                              child: Container(
                                child: Text(_npSpotOrderHisList![index].volumeTotalOriginal!,style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(0, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                            ...
                            Expanded(
                              child: Container(
                                child: Text(_findTrans(_npFuturesTranslate!, "OrderStatus", _npSpotOrderHisList![index].orderStatus!),style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(0, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                            ...
                            Expanded(
                              child: Container(
                                child: Text(_npSpotOrderHisList![index].insertDate! + ' ' + _npSpotOrderHisList![index].insertTime!,style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(0, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                            ...
                            Expanded(
                              child: Container(
                                child: Text(_npSpotOrderHisList![index].statusMsg!,style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(0, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                    ...
                    itemCount: _npSpotOrderHisList!.length,
                  ),
                  header: ClassicalHeader(
                    bgColor: Colors.transparent,
                    textColor: Colors.grey,
                  ...
                  onRefresh: () async {
                    await Future.delayed(const Duration(milliseconds: 500), () {
                      if (mounted) {
                        _npSpotOrderInfoHisPage = 1;
                        _NpSpotOrderHisDataHttp();
                      }
                    });
                  },
              ...
              _selectAssetEntrustmentType == '历史成交' ? Container(
                ...
                child: EasyRefresh(
                  ...
                  child:ListView.builder(
                      ...
                      return Container(
                        ...
                        child: Row(
                          children: [
                            Expanded(
                              child: Container(
                                child: Text(_npSpotTradeList![index].investorID!,style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(10, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                            ...
                            Expanded(
                              child: Container(
                                child: Text(_npSpotTradeList![index].exchangeID!,style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(0, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                            ...
                            Expanded(
                              child: Container(
                                child: Text(_npSpotTradeList![index].instrumentID!,style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(0, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                            ...
                            Expanded(
                              child: Container(
                                child: Text(_npSpotTradeList![index].tradeID!.trim(),style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(0, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                            ...
                            Expanded(
                              child: Container(
                                child: Text(_findTrans(_npFuturesTranslate!, "Direction", _npSpotTradeList![index].direction!),style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(0, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                            ...
                            Expanded(
                              child: Container(
                                child: Text( _npSpotTradeList![index].price!,style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(0, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                            ...
                            Expanded(
                              child: Container(
                                child: Text( _npSpotTradeList![index].volume!,style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(0, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                            ...
                            Expanded(
                              child: Container(
                                child: Text(_npSpotTradeList![index].tradeDate! + " " + _npSpotTradeList![index].tradeTime!,style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(0, 5, 10, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                    ...
                    itemCount: _npSpotTradeList!.length,
                  ),
                  header: ClassicalHeader(
                    bgColor: Colors.transparent,
                    textColor: Colors.grey,
                  ...
                  footer: _npSpotTradehasNextPage!
                      ? ClassicalFooter(
                    bgColor: Colors.transparent,
                    textColor: Colors.grey,
                    loadText: '上拉刷新',
                  ...
                  onRefresh: () async {
                    await Future.delayed(const Duration(milliseconds: 500), () {
                      if (mounted) {
                        _npSpotTradePage = 1;
                        _NpSpotTradeDataHttp();
                      }
                    });
                  },
                  onLoad: _npSpotTradehasNextPage!
                      ? () async {
                    await Future.delayed(const Duration(milliseconds: 500), () {
                      if (mounted) {
                        _npSpotTradePage += 1;
                ...
                child:EasyRefresh(
                  child:ListView.builder(
                    ...
                    itemBuilder: (context,index){
                      ...
                      if(_npSpotInvestorPositionList![index].posiDirection! == '1'){
                        _todoSell = '卖出';
                        _todoSellColor = Colors.white;
                      }else if(_npSpotInvestorPositionList![index].posiDirection! == '2'){
                        _todoSell = '卖出';
                        _todoSellColor = APP_MainRedColor;
                      }else if(_npSpotInvestorPositionList![index].posiDirection! == '3'){
                        _todoSell = '卖出';
                      ...
                      // print(_npSpotInvestorPositionList![index].lastPrice!);
                      String _positionProfit = ''; //持仓盈亏
                      if(neverDoubleTryOrZero(_npSpotInvestorPositionList![index].position!) != 0.0){
                        if(neverDoubleTryOrZero(_npSpotInvestorPositionList![index].lastPrice!) != 0.0){
                          double pos = (neverDoubleTryOrZero(_npSpotInvestorPositionList![index].lastPrice!) * neverDoubleTryOrZero(_npSpotInvestorPositionList![index].position!)) - neverDoubleTryOrZero(_npSpotInvestorPositionList![index].positionCost!);
                      ...
                      return Container(
                        ...
                        child: Row(
                          children: [
                            Expanded(
                              child: Container(
                                child: Text(_npSpotInvestorPositionList![index].investorID!,style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(10, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                            ...
                            Expanded(
                              child: Container(
                                child: Text(_npSpotInvestorPositionList![index].exchangeID!,style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(0, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                            ...
                            Expanded(
                              child: Container(
                                child: Text(_npSpotInvestorPositionList![index].instrumentID!,style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(0, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                            ...
                            Expanded(
                              child: Container(
                                child: Text(_findTrans(_npFuturesTranslate!, "PosiDirection", _npSpotInvestorPositionList![index].posiDirection!),style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(0, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                            ...
                            Expanded(
                              child: Container(//PositionDate
                                child: Text(_findTrans(_npFuturesTranslate!, "PositionDate", _npSpotInvestorPositionList![index].positionDate!),style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(0, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                            ...
                            Expanded(
                              child: Container(
                                child: Text(_npSpotInvestorPositionList![index].position!,style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(0, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                            ...
                            Expanded(
                              child: Container(
                                child: Text(_npSpotInvestorPositionList![index].positionCost!,style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                                padding: const EdgeInsets.fromLTRB(0, 5, 0, 5),
                                alignment: Alignment.centerLeft,
                              ),
                              flex: 2,
                            ...
                            // Expanded(
                            //   child: Container(
                            //     child: Text(_npSpotInvestorPositionList![index].useMargin!,style: const TextStyle(fontSize: 11,fontWeight: FontWeight.w500,color: Colors.white),),
                            //     padding: const EdgeInsets.fromLTRB(0, 5, 0, 5),
                            //     alignment: Alignment.centerLeft,
                            //   ),
                            //   flex: 2,
                                ...
                                onTap: (){
                                  AwesomeDialog(
                                    ...
                                    body: _TransactionPanelSignalWidget(
                                      _npSpotInvestorPositionList![index],
                                      todoSell:_todoSell,
                                    ),
                                  ).show();
                                },
                    ...
                    itemCount: _npSpotInvestorPositionList!.length,
                  ),
                  header: ClassicalHeader(
                    bgColor: Colors.transparent,
                    textColor: Colors.grey,
                  ...
                  footer: _npSpotInvestorPositionhasNextPage!
                      ? ClassicalFooter(
                    bgColor: Colors.transparent,
                    textColor: Colors.grey,
                    loadText: '上拉刷新',
                  ...
                  onRefresh: () async {
                    await Future.delayed(const Duration(milliseconds: 500), () {
                      if (mounted) {
                        _npSpotInvestorPositionPage = 1;
                        _NpSpotInvestorPositionDataHttp();
                      }
                    });
                  },
                  onLoad: _npSpotInvestorPositionhasNextPage!
                      ? () async {
                    await Future.delayed(const Duration(milliseconds: 500), () {
                      if (mounted) {
                        _npSpotInvestorPositionPage += 1;
