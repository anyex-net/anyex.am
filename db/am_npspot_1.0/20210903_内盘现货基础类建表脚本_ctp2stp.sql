...
drop table if exists NpSpotBroker;
create table NpSpotBroker
(
...
drop table if exists NpSpotExchange;
create table NpSpotExchange
(
    id                bigint                            not null comment '主键' primary key,
    exchangeID        varchar(10)                       not null comment '交易所代码',
    exchangeName      varchar(64)                                comment '交易所名称',
    exchangeProperty  varchar(8)                                 comment '交易所属性(0正常、1根据成交生成报单)',
...
/*
drop table if exists NpfuturesExchangeProduct;
create table NpfuturesExchangeProduct
(
    id                    bigint                            not null comment '主键' primary key,
...
) comment '内盘期货交易所期货产品表';
*/

drop table if exists NpSpotExchangeInstrument;
create table NpSpotExchangeInstrument
(
    id                      bigint                           not null comment '主键' primary key,
    ...
    volumeMultiple          int(11)                          not null comment '合约数量乘数',
    priceTick               decimal(22, 8)                   not null comment '最小变动价位',
    createDate              varchar(10)                               comment '创建日',
    ...
    enableStatus            varchar(16)    default 'disable' not null comment '启用状态(启用enable、不启用disable)',
    unique index (`instrumentID`, `exchangeID`, `productID`)
) comment '内盘现货交易所证券信息表';

drop table if exists NpSpotDepthMarketData;
create table NpSpotDepthMarketData
(
    id                   bigint                         not null comment '主键' primary key,
...
drop table if exists NpSpotExchangeTradeDay;
create table NpSpotExchangeTradeDay
(
    id                      bigint                        not null comment '主键' primary key,
    exchangeID              varchar(10)                   not null comment '交易所代码',
    productID               varchar(32)                   not null comment '产品代码',
